...
if (any(!is.na(genus_groups$med_growth))) {
  genus_groups%>%
    ...
    knitr::kable(
      col.names = paste(
        "Group", seq_along(unique(drop_na(genus_groups)$gen_group))
      ), row.names = FALSE
    )
}
```
...
data_cor <- data_cor%>%
  ...
  # cannot be lower than -2 cm/year
  ungroup()%>%
  mutate(
    dgrowth_lower = max(-2, quantile(diam_growth, 0.005, na.rm = TRUE)),
    # define general upper quantile for unidentified individuals
    dgrowth_upper = min(5, quantile(diam_growth, 0.995, na.rm = TRUE))
  )%>%
  # upper diameter growth threshold defined by species and size
  # cannot be higher than 5 cm/year
  group_by(diam_group)%>%
  mutate(dgrowth_upper = ifelse(!is.na(gen_group),
                                min(5, quantile(diam_growth, 0.995, na.rm = TRUE)),
                                dgrowth_upper
  ))%>%
...
neutral_div_sp <- data_cor%>%
  ...
  summarise(
    ...
    diversity_q0_sp_rar = Diversity(count, q = 0, Level = 100),
...
neutral_div_gen <- data_cor%>%
  ...
  summarise(
    ...
    diversity_q0_gen_rar = Diversity(count, q = 0, Level = 100),
...
if (params$site %in% c("mbaiki", "paracou", "suas")) {
  data_temp <- data_cor%>%
...
We also calculate the stem distribution by diameter class: 10-15 cm, 15-20 cm, 20-25 cm, 25-30 cm, 30-40 cm, 40-50 cm, 50+ cm. 
...
#| warning: false
data_cor <- data_cor%>%
  mutate(diam_class = cut(diameter_cor,
                          c(seq(10, 30, 5), 40, 50, 1000),
                          include.lowest = TRUE
  ))
data_aggr <-
  data_cor%>%
  filter(!is.na(diameter_cor))%>%
  group_by(Site, Plot, Year = year_census, diam_class)%>%
  summarise(
    nstem = n() / unique(PlotArea)
  )%>%
  pivot_longer(nstem, names_to = "variable")%>%
  mutate(variable = paste(variable, diam_class, sep = "_"))%>%
  select(-diam_class)%>%
  rbind(data_aggr)
```
...
data_aggr <-
  data_cor %>%
  filter(diameter_cor >= 50) %>%
  group_by(Site, Plot, Year = year_census) %>%
  summarise(ba50 = sum(ba / PlotArea)) %>%
  pivot_longer(ba50, names_to = "variable") %>%
  rbind(data_aggr)
```
...
agb_func <- function(d, wd, e_var) {
  exp(-2.023977 - 0.89563505 * e_var + 0.92023559 * log(wd) +
        2.79495823 * log(d) - 0.04606298 * (log(d)^2)) / 1000
}
data_cor <- data_cor%>%
  mutate(
...
data_aggr <- data_cor %>%
  filter(!is.na(diameter_cor))%>%
  group_by(Site, Plot, Year) %>%
  mutate(min_dbh = min(diameter_cor)) %>%
  ungroup() %>%
  mutate(recr = ifelse(
    year_census == recr_year &
      (diameter_cor - min_dbh)/(year_census - prev_census) < dgrowth_upper,
...
data_aggr <- data_cor %>%
  ...
  mutate(recr = ifelse(
    year_census == recr_year &
      # don't count large stems (that would have grown more than expected) as recruits
      (diameter_cor - min_dbh)/(year_census - prev_census) < dgrowth_upper,
    agb / (year_census - prev_census), 0
  )) %>%
  group_by(Site, Plot, Year = year_census, diam_class)%>%
...
data_aggr <- data_cor%>%
  subset(Year == mort_year)%>%
  group_by(Site, Plot, Year = year_census, next_census, diam_class)%>%
  summarise(agb_mort = sum(agb / (next_census - Year) / PlotArea))%>%
  mutate(Year = next_census)%>%
  subset(!is.na(Year))%>%
  select(Site, Plot, Year, diam_class, agb_mort)%>%
  pivot_longer(agb_mort, names_to = "variable")%>%
  mutate(variable = paste(variable, diam_class, sep = "_"))%>%
  select(-diam_class)%>%
  rbind(data_aggr)
